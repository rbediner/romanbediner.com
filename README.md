...
- **Homepage**: Credential eyebrow replaced by `AI-ENABLED OPERATING SYSTEMS` and an `.experience-logo-row` for selected operating experience logos (Disney, AWS, Laser Light Communications, Agentic Society). The logo band uses a top divider, centered logo alignment, added breathing room above/below the logos, and optically normalized logo heights (Laser Light enlarged, Agentic Society reduced for balance). The supporting "Selected leadership…" qualifier sentence under the logos has been removed. The `.home-primary-cta` About button was removed from the hero — About is accessible via the bottom nav only. The hero head carries modest `padding-bottom` for breathing room above the copy/photo row.
