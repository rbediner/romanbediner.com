...
- **Homepage**: Credential eyebrow replaced by `AI-ENABLED OPERATING SYSTEMS` and an `.experience-logo-row` for selected operating experience logos (Disney, AWS, Laser Light Communications, Agentic Society). The logo band uses a top divider, centered logo alignment, and tighter photo proportions. The `.home-primary-cta` About button has been removed from the hero — About is accessible via the bottom nav only.
...
- **Services page**: Simplified to four narrative `.svc-entry` blocks plus a dedicated `AI-ENABLED OPERATING SYSTEMS` framing section and an `APPLYING THE WORK` close. The old impact-card treatment is retired. The `section-eyebrow` "SERVICES" label has been removed — the H1 alone is sufficient.
...
- **Framework/brief mobile pills**: `.framework-progress-markers` uses `overflow-x: auto` + `flex-wrap: nowrap` + `scrollbar-width: thin` on mobile, with explicit trailing padding / scroll padding so all 6 stage pills remain reachable without clipping at the right edge. Framework CSS cache-bust at `?v=20260425a`.
