...
- Post-deploy browser smoke now ignores the known `static.cloudflareinsights.com` CSP-block console noise and `ERR_NAME_NOT_RESOLVED` failures for third-party analytics endpoints (GA DNS gap in headless Playwright environments). Real application runtime errors still fail the release.
