...
- **Agentic AI Employees resource** (`/resources/agentic-ai-employees/`): the flagship autonomous-organization reference architecture and first-party build report. It presents five employees: Project Manager, Chief of Staff, Director of Fleet Orchestration & Engineering, Continuous Improvement Engineer, and independent Staff Engineer, with an org chart, explicit autonomy boundaries, the brain/body split, a four-layer architecture, three closed operating loops, scheduled-run lifecycle, concrete model-routing tiers, Anthropic cost/reporting reliability loop, and a self-engineering path from queue to verified production. It does not claim an external customer case study without a named context and measured outcome. The page opens with the shared resource banner and a practical `Who This Is For` card for founders, CEOs, business owners, operators, AI builders, and implementation partners, then uses a compact orientation rail plus a public-facing minimal-build recipe. Its downloadable ten-page **Agentic Operations Architecture** is a unified artifact surface: `Preview the architecture` precedes `Download the architecture`, the preview stays collapsed until requested and expands inside the same surface, and both actions retain the shared resource analytics context and canonical file path. The artifact heading uses the same shared serif treatment as the surrounding editorial headings. Long-form navigation keeps `On this page` available immediately on desktop and mobile, with a fixed, compact placement that prevents it from covering the thesis or hero callout. All in-page targets reserve space for the sticky mobile header. Components remain scoped under `.resource-fleet-main` in `styles/resources.css`; the visual system uses whiteboard paper, pale grid, blue/red/gold semantics, shared framed diagram surfaces, captions, fullscreen controls for all ten diagram surfaces (SVG and HTML/CSS), and responsive touch-safe layouts, including a tablet breakpoint that prevents loop-card overflow and mobile spacing that keeps the architecture stamp clear of the closing caption. The canonical diagram source artifacts remain `assets/resources/agentic-ai-employees/fleet-system.svg`, `assets/resources/agentic-ai-employees/fleet-lifecycle.svg`, and `assets/resources/agentic-ai-employees/fleet-deploy.svg`; the page keeps the diagrams inline for CSP-safe typography and zoom behavior. SEO targets agentic AI employees, autonomous AI software engineering, AI agents that merge pull requests, autonomous code review, multi-agent orchestration, model routing, Anthropic cost reporting, and AI agents that deploy to production. The mobile orientation control is scoped to the resource stylesheet so the homepage does not pay for page-specific layout rules. The canonical URL and resource-hub placement are unchanged.
...
- Resources presentation contract:
  ...
  - `/resources/` includes the complete ten-page preview and download CTA for `Agentic Operations Architecture`; the preview is rendered from the approved canonical PDF and carries the `agentic-operations-architecture` GA4 resource context.
  - `/resources/agentic-ai-employees/` includes one unified Agentic Operations Architecture card with a collapsed, reusable ten-page preview pill followed by the primary Download the architecture pill. Both download CTAs use the shared `resource_pdf_download` event contract with `file_path` set to `/assets/downloads/agentic-operations-architecture-roman-bediner.pdf`.
  - Artifact naming is content-led: reserve `brief` for genuinely concise summaries. A substantive case-study or reference architecture uses its truthful title, such as `Agentic Operations Architecture`.
  - Its shared carousel runtime uses `resources-carousel.js?v=20260719`, so a browser with an older cached runtime still receives the correct `Page 1 of 10` PDF-preview language and navigation. Artifact QA must open the collapsed preview, advance one page, verify the canonical download target, and check no horizontal overflow at desktop and 390px phone width.
