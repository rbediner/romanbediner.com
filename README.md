...
- `/llms.txt` states the site’s public purpose, when agents should use it, its canonical routes, the `/connect/` form as the contact workflow, and public-safety boundaries. `robots.txt` advertises the guide.
- `/privacy/` is the substantive public privacy trust anchor. `/connect/` remains the canonical contact route and publishes `ContactPage` structured data. The homepage Organization schema uses that form as its approved `contactPoint` and records only the public regional address (`Raleigh, NC, US`); do not add an email, phone number, or street address without Roman’s explicit approval.
- `404.html` retains a real HTTP 404 and directs people and agents to the sitemap and agent guide. The sitemap includes the privacy page.
- `agent-access/` contains the source-controlled Cloudflare Worker that performs `Accept: text/markdown` negotiation. Its staging environment uses a dedicated `workers.dev` URL; production routing passes ordinary requests to Cloudflare's configured origin and returns Markdown only when explicitly requested. The Worker preserves HTTP 404 for unknown Markdown routes while providing sitemap and agent-guide recovery links, returns `Vary: Accept, Accept-Encoding`, and keeps Markdown responses out of cache until cache rules are separately reviewed.

## Typography & Design System (2026 Redesign)
- **Contained surfaces**: `docs/design-system/card-surfaces/README.md` is the canonical taxonomy for card-like UI, its approved visual references, shared tokens, interaction rules, route inventory, accessibility, and intentional exceptions. Read it before adding or changing a contained surface. The system has five base types only: Compact, Standard Content, Editorial Feature, Static Callout, and Form/Input. Interaction is a modifier. Static surfaces do not lift or use a pointer cursor; existing approved icons and semantic colors are preserved without adding decorative duplicates. The elevated editorial direction uses Roman’s blue-only soft edge washes, serif titles, compact markers, and generous spacing. It does not introduce teal, new decorative icons, deep shadows, or a dense SaaS-card wall.
- **Editorial card anatomy**: Compact question cards use concise prompt surfaces; Standard cards use a serif title, short blue rule, and readable explanation; Editorial cards preserve their approved existing icon where one exists, plus a semantic label and serif title; Static Callouts use a single calm blue-washed synthesis surface; Form surfaces use a compact all-caps label and structured controls. Dashboard questions/quadrants/views, PasteFlow capabilities, resource entries, Framework stages, About chapters, Services entries, Insights briefs, resource callouts, and Connect all follow this structure at the appropriate density. Decorative card numbers are deliberately excluded; where repeated content needs recognition, a unique, purposeful line icon corresponds to the capability or operating area, never a duplicated or generic filler symbol.
- **Resource metadata pills**: `.resource-meta` is a shared interface primitive, not route-specific decoration: every resource pill uses a 34px fixed visual height, 14px horizontal padding, a fully rounded radius, and 12px centered line-height. Labels may vary only in width; mobile and desktop must preserve the same vertical rhythm.
- **Brand lockup and CTAs**: Every visible `Bediner Advisory LLC` rendering uses the approved lockup—ink-black `Bediner`, blue `Advisory`, and a small ink-black superscript `LLC` immediately after the `y`; structured data and metadata keep the canonical plain legal name. Primary blue resource CTA pills share a 48px height, radius, type, and padding while retaining label-driven widths. Supporting-resource metadata pills sit apart from their serif headings rather than visually touching them.
...
- **Services page**: Simplified to five narrative `.svc-entry` blocks plus a dedicated `AI-ENABLED OPERATING SYSTEMS` framing section and an `APPLYING THE WORK` close. The old impact-card treatment is retired. Each service is a static editorial panel with its own preserved existing icon in a restrained blue-tinted tile, a compact number marker, and no false click affordance. The `section-eyebrow` "SERVICES" label has been removed — the H1 alone is sufficient. The final CTA pair establishes hierarchy: `Start a Conversation` (`.svc-cta-primary`) is primary and appears first; `Explore the Framework` (`.svc-cta-secondary`) is visually de-emphasized. Labels and destinations are unchanged.
- **About page**: Rebuilt around `OPERATING BACKGROUND`, a chapter-nav jump list, five operating chapters, and a simplified single-column `OPERATING PHILOSOPHY` close. The professional background uses five restrained editorial panels with explicit chapter numbers, attached role/company labels, readable narrative measures, and current-to-history ordering; the obsolete timeline rail and decorative orbs are removed. The visible chapter-nav chips were removed in favor of the floating "On this page" control (`section-nav.js`); the `.about-chapter-nav` markup is retained but visually hidden (`.sr-only`) so it still serves as that control's `[data-section-nav]` source (and an SR landmark) — the chapter sections keep `scroll-margin-top` for the fixed-header offset. The Operating Philosophy close uses a thin blue left-rule inset (`.about-philosophy-copy`) and a 1010px desktop paragraph measure aligned to the chapter narrative rail, with a full-width mobile fallback. The final CTA reads `Explore Service Models` and links to `/services/`.
- **July 2026 current-responsibility positioning**: The homepage, About, Services, Framework, Resources, and Connect surfaces now include neutral public-safe language for Chief Fractional Integration Officer work with NC Courage, while preserving Laser Light Communications as operating-architecture work and Agentic Society as a core AI-architecture and agentic-operating-systems proof point. About now has a fifth `FRACTIONAL INTEGRATION LEADERSHIP` chapter in the current-to-history editorial panel sequence; the obsolete timeline rail and decorative orbs are removed. In the About hero, the three current role/company relationships are semantic reading units: they form a calm grouped list on desktop and mobile, then leave a clear pause before the Bediner Advisory relationship statement. The homepage selected-experience band now presents six color marks: Disney+, AWS, Laser Light Communications, Agentic Society, NC Courage, and the user-supplied historical Omnigon OG mark; the manifest records each source and display treatment. SEO titles, descriptions, Open Graph/Twitter metadata, homepage Person schema, and page-level structured-data terms now cover Chief Fractional Integration Officer, fractional integration leadership, AI architecture, technology integration, and vendor evaluation. Confidential portal findings, vendor judgments, stakeholder names, and internal systems remain outside the public site. A public-safe NC Courage resource/case-study page is explicitly deferred to Phase 2.
...
- **Connect page**: Hero copy is conversation-first. The contact form is the primary direct contact method and is the first actionable element after the intro. The approved public fallback is `roman@romanbediner.com`, rendered as a quiet email link rather than a second action card. The retained LinkedIn executive-action block follows the form. The black `Send message` button is unchanged.
...
- **Resources hub**: redundant `<p class="resources-label">RESOURCES</p>` eyebrow removed — the H1 "Resources" alone is sufficient. The H1 renders all-caps (`.resources-main h1 { text-transform: uppercase }`) to match the `SERVICES` page-title treatment (rendering only; copy unchanged). The Agentic AI Employees card owns its flagship artifact: it calls the work a `Flagship build report`, keeps `Preview the field guide` before the canonical download, and remains closed until opened inside the card. Preview images are rendered from the downloaded PDF and their hashes are locked in `assets/resources/agentic-fleet-control-plane/preview-manifest.json`. This is the hub's single flagship card; Framework Summary, Dashboard, and PasteFlow use compact desktop split rows and single-column phone stacks so resource discovery stays scannable without repeating oversized card chrome.
...
- **Agentic AI Employees resource** (`/resources/agentic-ai-employees/`): the flagship autonomous-organization build report from Roman Bediner's Fractional COO work with Agentic Society. It presents eight named employees: Project Manager, Chief of Staff, Director of Fleet Orchestration & Engineering, Continuous Improvement Engineer, independent Staff Engineer, RevOps Engineer, AI Correspondent, and Solution Architect. The public view covers the org chart, autonomy boundaries, brain/body split, four-layer architecture, three closed operating loops, scheduled-run lifecycle, model-routing tiers, cost/reporting reliability loop, and a self-engineering path from queue to verified production. It does not expose private routes, identifiers, schedules, contacts, or operating internals. Its downloadable six-page **Agentic Fleet Control Plane** is the single public field guide: `Preview the field guide` precedes `Download the field guide`, the preview stays collapsed until requested and expands inside the same surface, and both actions retain the shared resource analytics context and canonical file path. The artifact heading uses the shared serif treatment; the roster uses a deliberate four-by-two desktop grid and a one-column phone stack. Long-form navigation keeps `On this page` available immediately on desktop and mobile. Components remain scoped under `.resource-fleet-main` in `styles/resources.css`; the visual system uses whiteboard paper, pale grid, blue/red/gold semantics, shared framed diagram surfaces, captions, fullscreen controls, and responsive touch-safe layouts. The canonical diagram source artifacts remain `assets/resources/agentic-ai-employees/fleet-system.svg`, `assets/resources/agentic-ai-employees/fleet-lifecycle.svg`, and `assets/resources/agentic-ai-employees/fleet-deploy.svg`; the page keeps the diagrams inline for CSP-safe typography and zoom behavior. SEO targets agentic AI employees, AI agent fleet, autonomous AI software engineering, AI agents that merge pull requests, autonomous code review, multi-agent orchestration, model routing, and AI agents that deploy to production. The canonical URL and resource-hub placement are unchanged.
- **Floating "On this page" section nav**: long editorial pages get a floating control (`/scripts/runtime/section-nav.js`) that is available at page load and opens a sheet of the same section links with active-section highlighting via `IntersectionObserver`. Progressive enhancement — the inline anchors work without JS, and the control simply never appears if JS/IO is unavailable. About reads its visible `.about-chapter-nav`; Services reads a visually-hidden `[data-section-nav]` listing its four `.svc-entry` sections (which now carry `id`s + `scroll-margin-top`). Styles live in `styles/site.css` (`.section-nav-fab`, `.section-nav-sheet`).
- **Resource detail visual family**: the Dashboard and PasteFlow detail pages share the Agentic AI Employees page's restrained reference-architecture treatment: a quiet radial field, oversized balanced title, gradient introduction surface, and large editorial lede, with route-specific artifact interactions preserved and mobile spacing scoped to each page.
...
- **Dashboard resource page**: six operating questions use compact contained prompts in a three-column desktop grid; the four dashboard-area cards and three core-view cards are content-sized editorial surfaces with dark-outline line icons, blue labels/rules, and no decorative numbers. The same compact card system is used by PasteFlow capabilities, preserving a readable one-column phone layout without gallery-sized empty space.
- `document.fonts.ready` added to `syncAboutTimelineOrbs()` to re-position timeline orbs after web font load.
- **Bediner Advisory LLC brand architecture (staging):** Roman Bediner remains the primary public brand and global header identity. The full public-page footer now adds the restrained line `Advisory services delivered through Bediner Advisory LLC.`; About states the founder/principal relationship, Services identifies the delivery entity, and Connect presents the approved Roman Bediner / Bediner Advisory LLC endorsement with ink-black `Bediner`, blue `Advisory`, and neutral `LLC`. Homepage, About, Services, and Connect structured data identify the LLC without replacing Roman as author or publisher. Framework, Resources, dashboard, primary navigation, and redirect-only routes remain Roman-led and outside the visible LLC-content scope. The contract is enforced by `QA/tests/test-llc-brand-architecture.js`.
- **Current-role SEO architecture:** The public positioning now names three current operating responsibilities consistently: Fractional COO for Agentic Society, Global Operations & Program Strategy Consultant for LaserLight Communications, and Chief Fractional Integration Officer for NC Courage. Exact role language is present in the homepage/About/Services/Connect copy and metadata; homepage Person/Organization entities and About ProfilePage markup reinforce the relationship without turning Roman's site into a company site. Sitemap `lastmod` values for the updated conversion/profile routes are refreshed to 2026-07-18.
- **Homepage operating-experience clarification (July 2026):** The longer operating-experience paragraph now names NC Courage's Chief Fractional Integration Officer leadership alongside LaserLight Communications and Agentic Society, so the role is visible in both the current-work callout and the supporting experience narrative.
...
- `/assets/downloads/agentic-fleet-control-plane-roman-bediner.pdf`
- `/assets/resources/agentic-fleet-control-plane/slides/`
- `/scripts/asset-generation/agentic-fleet-control-plane/generate_agentic_fleet_control_plane.py`
...
- Resources presentation contract:
  ...
  - `/resources/` includes the complete six-page preview and download CTA for `Agentic Fleet Control Plane`; the preview is rendered from the approved canonical PDF and carries the `agentic-fleet-control-plane` GA4 resource context.
  - `/resources/agentic-ai-employees/` includes one unified Agentic Fleet Control Plane card with a collapsed, reusable six-page preview pill followed by the primary Download the field guide pill. Both download CTAs use the shared `resource_pdf_download` event contract with `file_path` set to `/assets/downloads/agentic-fleet-control-plane-roman-bediner.pdf`.
  - Artifact naming is content-led: reserve `brief` for genuinely concise summaries. A substantive public field guide uses its truthful title, such as `Agentic Fleet Control Plane`.
  - Its shared carousel runtime uses `resources-carousel.js?v=20260719`, so a browser with an older cached runtime still receives the correct `Page 1 of 6` PDF-preview language and navigation. Artifact QA must open the collapsed preview, advance one page, verify the canonical download target, and check no horizontal overflow at desktop and 390px phone width.
