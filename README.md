...
- **About page**: Rebuilt around `OPERATING BACKGROUND`, a chapter-nav jump list, four operating chapters, and a simplified single-column `OPERATING PHILOSOPHY` close. The timeline renders four orbs (`.orb-1` through `.orb-4`). Chapter index items are functional same-page anchors styled as blue link-icon **chips** (bordered pill + leading link glyph via CSS mask) so they read as interactive; `scroll-margin-top` offsets the fixed header; hover/focus states added. The `.arc-item` label column is widened to 360px and `.era-title` set to 18px (16px mobile) so the longest heading (GLOBAL INFRASTRUCTURE ADVISORY) stays on a single line rather than wrapping; `.arc-item` adds left padding to clear the rail. The Operating Philosophy close uses a thin blue left-rule inset (`.about-philosophy-copy`). The final CTA reads `Explore Service Models` and links to `/services/`.
...
- **Homepage mobile logo sizing**: in the narrow two-column grid the operating-experience logos are sized by capped height + width (`width/height: auto`, per-mark `max-height`) so bold/wide marks (AWS, Agentic Society) do not optically dominate the compact ones (Disney). Optical balance, not literal parity.
