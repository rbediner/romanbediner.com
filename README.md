...
- `agent-access/` contains the source-controlled Cloudflare Worker that performs `Accept: text/markdown` negotiation. Its staging environment uses a dedicated `workers.dev` URL; production routing is defined but must not be deployed without explicit approval. The Worker returns `Vary: Accept, Accept-Encoding` and keeps Markdown responses out of cache until cache rules are separately reviewed.
