...
- **Editorial card anatomy**: Compact question cards use a small sequence marker plus concise prompt; Standard cards use a compact marker, serif title, short blue rule, and readable explanation; Editorial cards preserve their approved existing icon where one exists, plus a semantic label and serif title; Static Callouts use a single calm blue-washed synthesis surface; Form surfaces use a compact all-caps label and structured controls. Dashboard questions/quadrants/views, PasteFlow capabilities, resource entries, Framework stages, About chapters, Services entries, Insights briefs, resource callouts, and Connect all follow this structure at the appropriate density. Where a repeated set needs visual recognition, it uses a unique, purposeful line-icon family: each icon corresponds to one capability or operating area, never a duplicated or generic filler symbol.
