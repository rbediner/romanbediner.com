...
- Resources presentation contract:
  ...
  - `/resources/agentic-ai-employees/` includes a compact direct-download CTA plus a collapsed, reusable eight-slide preview for the same canonical architecture PDF. Both download CTAs use the shared `resource_pdf_download` event contract with `file_path` set to `/assets/downloads/agentic-operations-architecture-roman-bediner.pdf`.
