...
- **About page**: Rebuilt around `OPERATING BACKGROUND`, a chapter-nav jump list, four operating chapters, and a simplified single-column `OPERATING PHILOSOPHY` close. The timeline renders four orbs (`.orb-1` through `.orb-4`). Chapter index items are functional same-page anchors (`scroll-margin-top` offsets the fixed header; hover/focus states added). Timeline labels (`.era-title`/`.era-sub`) wrap instead of `nowrap` so long headings (e.g. GLOBAL INFRASTRUCTURE ADVISORY) stay inside the left label column and never cross the rail; `.arc-item` adds left padding to clear the rail. The Operating Philosophy close uses a thin blue left-rule inset (`.about-philosophy-copy`). The final CTA reads `Explore Service Models` and links to `/services/`.
- **Framework mobile stage nav**: `.framework-progress-markers` collapses to a two-column grid on mobile (no horizontal scroll), keeping all six stage pills reachable with full-width tap targets; markers `justify-self: stretch` so each pill fills its cell and cannot overlap the adjacent column; connecting dots are hidden on mobile with the desktop rail. Framework CSS cache-bust at `?v=20260627a`.
