...
- **Agentic AI Employees resource** (`/resources/agentic-ai-employees/`): an evergreen reference-architecture page (brain/body split, the doer/operator roster, the scheduled-run lifecycle, the operating layer, principles, and the Git-driven deploy model) targeting the "agentic AI employees / autonomous AI agent fleets" search intent. Components are scoped under `.resource-fleet-main` in `styles/resources.css` (`.fleet-split`, `.fleet-grid`/`.fleet-agent`, `.fleet-steps`, `.fleet-feature-grid`, `.fleet-principles`, `.fleet-pill`) and collapse to one column at ≤767px. Diagrams are static hand-built inline **SVG** (`.fleet-diagram`, scrolls inside its frame on mobile via `overflow-x:auto` + `min-width`) plus the `.fleet-split` CSS block — no runtime JS, CSP-safe. Sections include the full stack ("How it's wired": Vercel cron runtime, the JS agent loop, integration clients, self-healing access — `.fleet-stack`), an Install &amp; setup how-to, and a Git→Vercel deploy pipeline with its own SVG. Evergreen = no model names and no costs, but the real stack (Vercel, GitHub, cron, JS, `vercel.json`) is named. Lead card on the Resources hub (intro copy enumerates "four forms"); in the sitemap.
