...
- **Framework mobile stage nav**: `.framework-progress-markers` collapses to a two-column grid on mobile (no horizontal scroll), keeping all six stage pills reachable with full-width tap targets; connecting dots are hidden on mobile with the desktop rail. Framework CSS cache-bust at `?v=20260627a`.
