...
- Resources presentation contract:
  ...
  - `/resources/agentic-ai-employees/` includes one unified Portable Architecture Brief card with a collapsed, reusable eight-slide preview pill followed by the primary Download the brief pill. Both download CTAs use the shared `resource_pdf_download` event contract with `file_path` set to `/assets/downloads/agentic-operations-architecture-roman-bediner.pdf`.
