...
- **Resources hub**: redundant `<p class="resources-label">RESOURCES</p>` eyebrow removed. The Agentic AI Employees card owns the flagship **Agentic Fleet Build Guide**: `Preview the build guide` precedes the canonical download, the 24 preview images are rendered from that PDF, and their hashes are locked in `assets/resources/agentic-fleet-build-guide/preview-manifest.json`. The card stays closed until a visitor requests it.
...
- **Agentic AI Employees resource** (`/resources/agentic-ai-employees/`): a flagship public build kit from Roman Bediner's Fractional COO work with Agentic Society. It presents eight named employees while keeping private routes, identifiers, schedules, contacts, and operating internals private. The 24-page **Agentic Fleet Build Guide** is the follow-along path: it starts with an autonomous-agent contract and moves through cloud runtime, cron, durable state, observability, recovery, and promotion. The paired **Agent Builder Starter Prompt** is the Session Zero handoff: a copy-ready, plain-English interview that creates the first implementation contract before the remaining sessions. The preview uses exact PDF-rendered pages, stays collapsed until requested, has a visible `Close preview` action in full screen, and preserves the shared resource analytics context. SEO targets agentic AI employees, agentic fleet build guide, cloud-first AI agents, shared memory, model routing, prompt caching, and autonomous code review.
- **Agentic diagram source assets**: `/assets/resources/agentic-ai-employees/fleet-system.svg`, `/assets/resources/agentic-ai-employees/fleet-lifecycle.svg`, and `/assets/resources/agentic-ai-employees/fleet-deploy.svg` remain the canonical vector sources for the inline diagrams on the flagship page.
...
- Resources presentation contract:
  ...
  - `/resources/` includes the complete 24-page preview and download CTA for `Agentic Fleet Build Guide`; the preview is rendered from the approved canonical PDF and carries the `agentic-ai-employees` GA4 resource context.
  - `/resources/agentic-ai-employees/` presents the linked build sequence explicitly: **Step 1** is the 24-page Agentic Fleet Build Guide (read pages 1–4 first); **Step 2** is the downloadable/copyable Agent Builder Starter Prompt, which begins Session Zero as a plain-English interview. The hub keeps that pair inside the one flagship card rather than creating redundant resource entries. The floating `On this page` control hides only while either phone-sized artifact panel is visible, so it never covers its primary action.
  - The canonical Markdown prompt is copyable on-page when JavaScript is available and still downloadable without JavaScript. It requires a one-job, one-source, one-output, one-approver MVP; a worked Morning Market Brief example; a practical file package; cloud routine; scoped credential, routing, caching, observability, and self-healing limits; and a first-production-run scorecard. The artifact is public-safe and does not expose internal fleet routes, credentials, or operating instructions.
  ...
  - Its shared carousel runtime uses `resources-carousel.js?v=20260824a`, so a browser receives the correct `Page 1 of 24` language, visible close control, and PDF-preview navigation. Artifact QA opens the collapsed preview, advances one page, confirms the full-screen exit, verifies the canonical download target, and checks no horizontal overflow at desktop and 390px phone width.
