...
- **Homepage**: Credential eyebrow replaced by `AI-ENABLED OPERATING SYSTEMS` and an `.experience-logo-row` for selected operating experience logos (Disney, AWS, Laser Light Communications, Agentic Society). The logo band uses a top divider, centered logo alignment, added breathing room above/below the logos, and bounded visual cells that normalize the marks by visible footprint. Official color marks are retained; transparent source padding is cropped visually so AWS and Laser Light remain legible, while the crest has a larger dedicated footprint. The supporting "Selected leadership…" qualifier sentence under the logos has been removed. The `.home-primary-cta` About button was removed from the hero — About is accessible via the bottom nav only. The hero head carries modest `padding-bottom` for breathing room above the copy/photo row.
