...
- **Agentic AI Employees resource** (`/resources/agentic-ai-employees/`): the flagship autonomous-organization reference page. It now presents five employees — Project Manager, Chief of Staff, Director of Fleet Orchestration & Engineering, Continuous Improvement Engineer, and independent Staff Engineer — with an org chart, explicit autonomy boundaries, the brain/body split, the system architecture, scheduled-run lifecycle, and a self-engineering loop from engineering queue to verified production. The page deliberately positions Slack as one working surface rather than the system itself. Components remain scoped under `.resource-fleet-main` in `styles/resources.css`; premium visual treatment includes the roster grid, dark org chart, autonomy matrix, engineering flow, and responsive diagram/lightbox behavior. The canonical diagram source artifacts remain `assets/resources/agentic-ai-employees/fleet-system.svg`, `assets/resources/agentic-ai-employees/fleet-lifecycle.svg`, and `assets/resources/agentic-ai-employees/fleet-deploy.svg`; the page keeps the diagrams inline for CSP-safe typography and zoom behavior. SEO targets agentic AI employees, AI agent fleets, autonomous code review, multi-agent orchestration, and AI agents that deploy to production. The canonical URL and resource-hub placement are unchanged.
