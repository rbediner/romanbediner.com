...
- **Homepage**: Credential eyebrow replaced by `AI-ENABLED OPERATING SYSTEMS` and an `.experience-logo-row` for selected operating experience logos (Disney, AWS, Laser Light Communications, Agentic Society). The logo band uses a top divider, centered logo alignment, added breathing room above/below the logos, and optically normalized logo heights — bold/wide marks (AWS, Agentic Society) held smaller and the compact Disney mark held larger so all five read at similar visual weight on desktop and the three-column mobile grid. The supporting "Selected leadership…" qualifier sentence under the logos has been removed. The `.home-primary-cta` About button was removed from the hero — About is accessible via the bottom nav only. The hero head carries modest `padding-bottom` for breathing room above the copy/photo row.
...
- **Homepage mobile logo sizing**: in the narrow six-track grid, three operating-experience logos fill the first row and Agentic Society plus the crest form a centered second-row pair. Marks are sized by capped height + width (`width/height: auto`, per-mark `max-height`) so bold/wide logos do not optically dominate compact ones, while the crest remains a restrained 64px. Optical balance, not literal parity.
...
- **Agentic AI Employees resource** (`/resources/agentic-ai-employees/`): an evergreen reference-architecture page (brain/body split, the doer/operator roster — **Agent — Project Manager**, **Agent — Chief of Staff**, **Agent — Orchestration Director** — the scheduled-run lifecycle, the operating layer, principles, and the Git-driven deploy model) targeting the "agentic AI employees / autonomous AI agent fleets" search intent. Components are scoped under `.resource-fleet-main` in `styles/resources.css` (`.fleet-split`, `.fleet-grid`/`.fleet-agent`, `.fleet-steps`, `.fleet-feature-grid`, `.fleet-principles`, `.fleet-pill`) and collapse to one column at ≤767px. Three polished, self-contained inline **SVG** diagrams (`.fleet-diagram`, each scrolls inside its frame on mobile via `overflow-x:auto` + `min-width`) carry the architecture: a system diagram ("How it's wired"), the scheduled-run lifecycle ("How one scheduled run executes", a seven-step rail above the prose steps), and the Git→Vercel deploy pipeline ("One git push is the whole deploy"). The source SVGs are kept as the canonical artifacts at `assets/resources/agentic-ai-employees/fleet-system.svg`, `assets/resources/agentic-ai-employees/fleet-lifecycle.svg`, and `assets/resources/agentic-ai-employees/fleet-deploy.svg`, and their markup is inlined into the page so the diagrams inherit DM Sans — no runtime JS, CSP-safe. The `.fleet-split` CSS block renders the brain/body split. Sections include the full stack ("How it's wired": Vercel cron runtime, the JS agent loop, integration clients, self-healing access — `.fleet-stack`) and an Install &amp; setup how-to. The overview `.fleet-pillrow` renders as an all-caps grid of six architecture properties (3 columns desktop, 2×3 on mobile). Each diagram has a tap-to-open full-screen lightbox (`/scripts/runtime/fleet-diagram-zoom.js`, native `<dialog>`, fit/zoom toggle, clones the inline SVG so DM Sans is preserved, CSP-safe with no inline script). The **roster** ("Two doers and an operator") leads the page (directly after the overview pills) so the reader meets the agents first, immediately followed by a **human-oversight** callout ("A human is always in charge"); the opening lede also states a person stays in charge of the fleet. This framing is deliberate — the agents are positioned as human-directed leverage (direct reports under a human owner who approves consequential moves), not job replacement. Evergreen = no model names and no costs, but the real stack (Vercel, GitHub, cron, JS, `vercel.json`) is named. The page eyebrow (`.resources-label`) reads **AVAILABLE NOW** (an availability badge). Lead card on the Resources hub (intro copy enumerates "four forms"); in the sitemap.
...
- **Homepage operating-experience logos**: uniform soft-graphite monochrome (`grayscale(1) brightness(0)`, opacity 0.6) with generous column spacing and a subtle hover lift to full ink — a restrained, premium "selected experience" row rather than heavy pure-black marks.
