...
- **Resources hub**: redundant `<p class="resources-label">RESOURCES</p>` eyebrow removed. The Agentic AI Employees card owns the flagship **Agentic Fleet Build Guide**: `Preview the build guide` precedes the canonical download, the twelve preview images are rendered from that PDF, and their hashes are locked in `assets/resources/agentic-fleet-build-guide/preview-manifest.json`. The card stays closed until a visitor requests it.
