...
- **Resources hub**: redundant `<p class="resources-label">RESOURCES</p>` eyebrow removed — the H1 "Resources" alone is sufficient. The H1 renders all-caps (`.resources-main h1 { text-transform: uppercase }`) to match the `SERVICES` page-title treatment (rendering only; copy unchanged). The Agentic AI Employees card owns its architecture artifact: it calls the work a `Reference architecture`, keeps `Preview the architecture` before the canonical download, and remains closed until opened inside the card. Preview images are rendered from the downloaded PDF and their hashes are locked in `assets/resources/agentic-operations-architecture/preview-manifest.json`.
