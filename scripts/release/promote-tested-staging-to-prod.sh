...
echo "[release] Verifying prod CI + deploy + live health for SHA ${STAGING_SHA}"
node scripts/release/verify-prod-release.js --branch prod --sha "${STAGING_SHA}" --timeout 3600 --interval 15
