/*
 * Purpose:
 * - Regenerate README framework deep-link section from canonical Framework stage sections.
 *
 * Architectural role:
 * - Keeps documentation synchronized with content slugs as part of QA and CI workflows.
 *
 * Dependencies:
 * - Node.js filesystem APIs and stable Framework section markup structure.
 *
 ...
 */
/**
 * Auto-generates direct anchor links for Framework stages in README.md.
 */
const fs = require('fs');
const path = require('path');

const ROOT = path.resolve(__dirname, '..', '..');
const INSIGHTS_PATH = path.join(ROOT, 'framework', 'index.html');
...
function parseInsights(html) {
  // Each framework section id and h2 pair is used to produce stable anchor links.
  const matches = [...html.matchAll(/<section id="([a-z0-9-]+)" class="framework-section insight-card">[\s\S]*?<h2>([^<]+)<\/h2>/g)];
...
function buildSection(entries) {
  ...
  for (const entry of entries) {
    ...
    lines.push(`https://romanbediner.com/framework/#${entry.slug}`);
    lines.push('');
  }
...
console.log(`Updated README framework links from ${entries.length} framework stage(s).`);
