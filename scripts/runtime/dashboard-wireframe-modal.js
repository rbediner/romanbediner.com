 ...
 *
 * Purpose: Opens/closes the wireframe preview modal on the AI-Enabled Operations Dashboard
 *   resource page. Injects image src lazily on first open to avoid loading the asset on every
 *   page view. Handles keyboard (Escape), backdrop click, and close button interactions.
 *
 ...
 *   The wireframe image src points to a same-origin static asset only.
 *
...
(function () {
  ...
  var image = modal ? modal.querySelector('[data-wireframe-src]') : null;

  if (!modal || !trigger || !image) return;

  var wireframeSrc = image.getAttribute('data-wireframe-src');
  var loaded = false;

  function openModal() {
    if (!loaded) {
      image.setAttribute('src', wireframeSrc);
      loaded = true;
    }
