...
if (!/\.experience-logo-cell-agentic\s*\{[\s\S]*?grid-column:\s*2\s*\/\s*span\s*2[\s\S]*?grid-row:\s*1/i.test(homeCss)) {
  failures.push('Agentic Society must lead the centered first mobile logo row.');
}
if (!/\.experience-logo-cell-courage\s*\{[\s\S]*?grid-column:\s*4\s*\/\s*span\s*2[\s\S]*?grid-row:\s*1/i.test(homeCss)) {
